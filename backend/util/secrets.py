...
if __name__ == "__main__":
    ...
    # Access values
    print(f"🌐 PocketBase URL: {secrets.pocketbase_url}")
    print(f"📧 Admin Email: {secrets.admin_email}")
    print(f"🔑 Admin Password: {'***' if secrets.admin_password else '❌ Not set'}")

    # Set a custom value
    secrets.set("custom.favorite_anime", "Made in Abyss")
    print(f"🎥 Custom value: {secrets.get('custom.favorite_anime')}")

    # Update multiple values at once
    secrets.update({
        "app": {
            "name": "Aina-chan's Awesome App",
            "debug": False,
        },
        "custom": {
            "theme": "dark",
    ...
    print(f"📱 App Name: {secrets.get('app.name')}")
    print(f"🎨 Theme: {secrets.get('custom.theme')}")
